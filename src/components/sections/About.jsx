            ...
            <section id="education" className="min-h-screen flex items-center justify-center pt-16 pb-8 md:py-16 lg:py-20 px-4 snap-start">
                <RevealOnScroll>
                    <div className="max-w-4xl mx-auto">
                        <h2 className="text-xl sm:text-2xl md:text-4xl font-bold mb-8 md:mb-16 lg:mb-20 bg-gradient-to-r from-yellow-500 to-purple-500 bg-clip-text text-transparent text-center flex items-center justify-center gap-2 md:gap-3">
                            <span className="text-xl sm:text-2xl md:text-4xl">🎓</span>
                            Education
                        </h2>
                        
                        <div className="p-4 md:p-6 lg:p-8 rounded-xl border-white/10 border hover:-translate-y-1 transition-all glass">
                            <div className="space-y-4 md:space-y-6 text-gray-300">
                                <div className="bg-black/20 rounded-lg p-4 md:p-6 border border-yellow-500/20">
                                    <div className="text-xl sm:text-2xl md:text-3xl font-bold text-white mb-3 md:mb-4">
                                        {skills.education?.course}
                                    </div>
                                    <div className="text-lg md:text-xl text-yellow-400 font-medium mb-3">
                                        {skills.education?.specialization}
                                    </div>
                                    <div className="text-base md:text-lg text-amber-300 font-medium mb-3 md:mb-4">
                                        {skills.education?.school}
                                    </div>
                                    <div className="text-base md:text-lg text-gray-400 flex items-center gap-2">
                                        <span className="text-lg md:text-xl">📅</span>
                                        {skills.education?.date}
                                    </div>
                                </div>
                                
                                <div className="bg-gray-900/40 rounded-lg p-4 md:p-6">
                                    <h4 className="text-fuchsia-400 font-semibold text-lg md:text-xl mb-4 md:mb-5 flex items-center gap-2">
                                        <span>📚</span>
                                        Relevant Coursework:
                                    </h4>
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
                                        {skills.education?.relevantCoursework?.map((course, index) => (
                                            <div key={index} className="bg-black/30 rounded-lg p-3 md:p-4 border border-purple-500/20">
                                                <span className="text-gray-200 text-base md:text-lg">{course}</span>
                                            </div>
                                        ))}
                                    </div>
                                </div>
